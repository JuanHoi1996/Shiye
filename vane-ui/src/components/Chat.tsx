...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      setInputDockHeight(el.offsetHeight);
    });
    ro.observe(el);
    setInputDockHeight(el.offsetHeight);
  ...
  useEffect(() => {
    ...
    if (messages.length === 1) {
      document.title = `${messages[0].query.substring(0, 30)} - 师爷 Shiye`;
    }
  ...
  return (
    <div
      className="flex flex-col space-y-6 pt-8 sm:mx-4 md:mx-8"
      style={{
        paddingBottom: `max(${inputDockHeight + 32}px, 7rem)`,
      }}
    >
