...
const CJK_TTF = '/fonts/NotoSansSC-400.ttf';
const CJK_FONT = 'NotoSansSC';
let cjkBase64: string | null = null;
let cjkLoadFailed = false;
...
function arrayBufferToBase64(buf: ArrayBuffer): string {
  ...
  for (let o = 0; o < bytes.length; o += chunk) {
    const slice = bytes.subarray(o, o + chunk);
...
/**
 * Load Noto Sans SC (OFL) once, embed in jsPDF for CJK. Falls back to Helvetica.
 */
async function tryEmbedCjkFont(doc: jsPDF) {
  if (cjkLoadFailed) {
    return;
  }
  if (cjkBase64 === null) {
    try {
      const res = await fetch(CJK_TTF);
      if (!res.ok) {
        throw new Error(`Font ${res.status}`);
      }
      cjkBase64 = arrayBufferToBase64(await res.arrayBuffer());
    } catch (e) {
      console.error('CJK font load failed, using default font', e);
      cjkLoadFailed = true;
      return;
    }
  }
  if (cjkBase64) {
    doc.addFileToVFS('NotoSansSC-400.ttf', cjkBase64);
    doc.addFont('NotoSansSC-400.ttf', CJK_FONT, 'normal', 'normal');
  }
}

const applyBodyFont = (doc: jsPDF) => {
  if (!cjkLoadFailed && cjkBase64) {
    doc.setFont(CJK_FONT, 'normal');
  } else {
    doc.setFont('helvetica', 'normal');
  }
};

const applyHeadingFont = (doc: jsPDF) => {
  if (!cjkLoadFailed && cjkBase64) {
    doc.setFont(CJK_FONT, 'normal');
  } else {
  ...
  sections.forEach((section) => {
    ...
    if (section.message.responseBlocks.length > 0) {
      ...
      const assistantLines = doc.splitTextToSize(
        assistantTextForPdfExport(section.message.responseBlocks),
        180,
      );
                  ...
                  <div className="p-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <button
                        className="w-full flex items-center gap-3 px-3 py-2 text-left rounded-xl hover:bg-light-secondary dark:hover:bg-dark-secondary transition-colors duration-200"
                        onClick={() => {
                          void exportAsPDF(sections, title || '');
                        }}
                      >
